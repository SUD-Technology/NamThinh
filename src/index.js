    ...
    handlebars.engine({
        ...
        helpers: {
            isAdmin: function (val, options) {
                if (val == 'admin') return options.fn(this);
            },
            isSale: function (val, options) {
                if (val == 'sale' || val == 'admin') return options.fn(this);
            },
            isAccountant: function (val, options) {
                if (val == 'accountant' || val == 'admin') return options.fn(this);
            },
...
app.get('/home', async (req, res, next) => {
    ...
    const partners = await Partners.find({}).then((ps) => {
        return ps.map((p) => {
            return {
                name: p.partner_name,
                image: p.image,
...
app.get('/policy/:slug', (req, res) => {
    ...
    Policy.findOne({ slug }).then((policy) => {
        if (policy) {
            let data = {
